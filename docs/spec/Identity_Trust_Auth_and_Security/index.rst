...
- Create a canonical document to sign:
  ...
  - Eliminate all objects that are generated by a node in the Learning Registry network, leaving only those objects that are supplied by the user . Objects to be eliminated include: doc_id, publishing_node, update_timestamp, `▲node_timestamp,`:changed: create_timestamp
