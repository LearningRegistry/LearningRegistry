...
class ModelParser(object):
    ...
    _VALUE_TYPE_OBJECT = 'valueTypeObject'
    ...
    VALUE = (SPEC_DATA_TYPES.setResultsName(_VALUE_TYPE)
             ...
             |INLINE_VALUE.setResultsName(_VALUE_TYPE_OBJECT)
             |OBJECT.setResultsName(_OBJECT).setResultsName(_VALUE_TYPE_OBJECT)
            )+ \
    ...
    def _extractObjectType(self, parseResults):
        ...
        if isinstance(parseResults, ParseResults) == False:
            return
        ...
        for key in parseResults.keys():
            ...
            obj = parseResults[key]
            if self._VALUE_TYPE_OBJECT in parseResults[key].keys():
                parseResults[key][self._VALUE_TYPE] = 'object'
                # Remove the old inline key.
                #parseResults[key].pop(self._VALUE_TYPE_OBJECT)
                
                # If the object is of type ParseResults process it futher to 
                # extract any comments or array info
            #self._processParseResults(obj)
            #parseResults[key] = obj.asDict()
    ...
    def _processParseResults(self, parseResults):
        """Process the pyparsing ParseResult to a more usable project"""
        self._extractObjectType(parseResults)
        self._extractArrayType(parseResults)
        self._cleanup(parseResults)
        self._extractCommentInfo(parseResults)
        self._extractIsRequiredIf(parseResults)
    ...
    def _extractData(self):
        ...
        self._modelInfo =  ModelParser.OBJECT.parseString(self._fileString)
